      ...
      <div className="space-y-6">
        {formData.companyCategory.map((category) => {
          ...
          return (
            <div key={category} className={`${colors.bg} rounded-lg p-3 ${colors.border} border`}>
                ...
                {sectors.map((sector) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          updateFormData({ sectorsServed: [...formData.sectorsServed, sector] });
                        } else {
                          updateFormData({ sectorsServed: formData.sectorsServed.filter(s => s !== sector) });
                        }
                      }}
                      className="sr-only"
                    />
              ...
              {/* Other input for sectors */}
              {formData.sectorsServed.includes('Other') && (
                <div className="mt-3">
                  <FormInput
                    label="Please specify other sectors (comma-separated)"
                    value={formData.sectorsOther || ''}
                    onChange={(value) => updateFormData({ sectorsOther: value })}
        ...
        {formData.sectorsServed.length > 0 && (
          <div className="bg-slate-50 rounded-lg p-2">
            ...
            {formData.sectorsOther && formData.sectorsOther.trim() && (
              <div className="mt-2">
                <h5 className="text-xs font-semibold text-slate-700 mb-1">Custom Sectors:</h5>
                <div className="flex flex-wrap gap-2">
                  {formData.sectorsOther.split(',').map((item, index) => {
